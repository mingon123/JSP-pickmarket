...
public class AdminReportDeleteAction implements Action{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ReportVO db_target = dao.getDetailReportByAdmin(report_num);

		// 회원정보 삭제
		dao.deleteReport(report_num);
		// 프로필 사진 삭제
		FileUtil.removeFile(request, db_target.getReport_img());
		
		request.setAttribute("notice_msg", "삭제가 완료되었습니다.");
		request.setAttribute("notice_url", request.getContextPath()+"/report/adminReportList.do");
